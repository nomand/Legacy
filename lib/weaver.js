...
function doStats(log)
{
    return `<div id="stats">
        ...
        <a id="stat" class="round" >${stats.num_tags + String(stats.num_tags>1 || stats.num_tags == 0 ? " tags":" tag")}</a>
    </div>`;
}
...
function doJournal(db)
{
    ...
    for(var i = lastpost + 1; i < db.length; i++)
    {
        if(db[i].term != "" && post < page)
        {
            posts += `
            <div id="entry">
                <a id="project" onClick="goToProject('${db[i].term}')">${db[i].term}</a><span id="date">${db[i].date.substr(0,2) + "." + db[i].date.substr(2,3)}</span>
                <div id="note">${db[i].note}</div>
                ${ db[i].pict != "" ? `<div id="pict" class="round" style="background-image: url('${checkImageFormat('content/images/'+db[i].pict)}');" onClick="showImage(${i})"></div>` : `` }
            </div>`;
            post++
            lastpost = i;
        }
...
function goToProject(term)
{
    document.getElementById("content").innerHTML = ``;
    let posts = ``;
    posts += doStats(DB);
    posts += `<div id="logheading">${"Log: " + term}</div>`;
    ...
    for(i in alphabet)
    {
        if(alphabet[i].length > 1)
        {
            ...
            for(j=1; j<alphabet[i].length; j++)
            {
                ...
                posts += `<a onClick="goToProject('${alphabet[i][j]}')" id="topic">${alphabet[i][j]}</a>`
            }
            posts += `</div>`
        }
    }
...
function checkImageFormat(path)
{
    ...
    for(i in extension)
    {
        image.src = path+extension[i];
        if (image.width != 0)
        {
            return path+extension[i];
        }
    }
}
