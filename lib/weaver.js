...
function doJournal(db)
{
    ...
    for(i in db)
    {
        if(db[i].term != "")
        {
            posts += `
            <div id="entry">
                <a href="#${String(db[i].term).to_url()}" id="project">${db[i].term}</a><a href="#${String(db[i].date).to_url()}" id="date">${db[i].date}</a>
...
function doTopics()
{
    ...
    for(i in DB)
    {
        let alpha = DB[i].term.substr(0,1);
        if(DB[i].term != "")
        {
            if(alphabet[alpha] == null)
                alphabet[alpha] = [alpha];

            if(alphabet[alpha][0] == alpha && alphabet[alpha].indexOf(String(DB[i].term)) === -1)
                alphabet[alpha].push(DB[i].term);
        }
    }
    ...
    //Object.keys(alphabet).sort(function(a, b) { return alphabet[0] > alphabet[0] ? 1 : -1; })
    console.log(Object.keys(alphabet)[0])
    return posts;
}
