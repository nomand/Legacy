    ...
    for(i in DB)
    {
        ...
        if(DB[i].term != "")
        {
            if(alphabet[alpha] == null)
            {
                ...
                posts += `<div id="topicalphabet">${alpha}</div>`
            }

            if(alphabet[alpha][0] == alpha)
            {
                if(alphabet[alpha].indexOf(String(DB[i].term)) === -1)
                {
                    alphabet[alpha].push(DB[i].term);
                    posts += `<a href="#${String(DB[i].term).to_url()}" id="topic">${DB[i].term}</a>`
                }
            }
        }
    }
