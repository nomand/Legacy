...
{
    html = document.body;
    let db = new Scribe(DATABASE.daily).parse();
    let view = ``;
    let posts = ``;
    ...
    function doStats(log)
    {
        ...
        <span id="stat" class="round">${stats.num_tags + String(stats.num_tags>1?" tags":" tag")}</span>
        </div>`;
    }
...
function showImage(log)
{
    document.body.innerHTML +=
    `<div class="lightbox">
        <div style="background-image: url('content/images/${this.db[log].pict}.jpg');"></div>
    </div>`;
}
