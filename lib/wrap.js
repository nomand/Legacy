function Wrap()
{
  ...
  {
    let dbKeys = Object.keys(this.database);
    var splitTarget = target.split(" ");
    if (splitTarget[0].toLowerCase() === 'tag')
    {
      console.log("looking at a tag: " + target)
      document.getElementById("content").innerHTML = window['tagTemplate'](this.database, target);
    }
    else
    {
      let template = this.database[Object.keys(this.database)[dbKeys.indexOf(target)]].VIEW;
  ...
  this.filter = function(target)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        for(i = 0; i < this.keys.length; i++)
        {
          if(this.keys[Object.keys(this.keys)[i]].toLowerCase() === splitTarget[0])
          {
            tempDatabase[this.keys[i]] = this.database[this.keys[i]];
          }
        }
