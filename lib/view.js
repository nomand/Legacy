function View()
{
  ...
  {
    let header = `
      <div id="header" style="background-image: url('assets/img/noise.png');">
  ...
  this.getTemplate = function(db)
  {
    ...
    let template = db[Object.keys(db)[0]].TMPL

    if(template !== 'undefined')
      this.grid.innerHTML = window[template + "Template"](dbKeys, template);
  ...
  {
    let menuContent = `<a id="stat" class="round" href="#">${value.total} Home</a>`;
    ...
    {
      menuContent += `<div style="display:inline-block;">${value.tags.length} tags:`;
      for (var t = 0; t < (SETTINGS.STATSNUMTAGS == 0 ? value.tags.length : Math.min(value.tags.length, SETTINGS.STATSNUMTAGS)); t++)
      {
        menuContent += `<a href='#tag-${value.tags[t][0]}' class="tag">${value.tags[t][0]} (${value.tags[t][1]})</a>`;
      }
      menuContent += `</div>`;
    }
