function MakeHeader(db, target)
{
    ...
    {
      console.log(target + "is a " + db[target].VIEW);
      return MakeFullScreenImage(db[target].HEAD);
    }
    else
    {
...
function MakeFullScreenImage(photo)
{
    console.log("building fullscreen");
    return html = `
    <div id="sense" style="background-image: url('content/images/${photo}')"></div>
    `
}
