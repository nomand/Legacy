function projectTemplate(db, target)
{
  ...
  if(typeof db[target].HEAD !== 'undefined')
  {
    if(db[target].HEAD.split("https").length > 1)
    {
      ...
      document.getElementsByName('og:image')[0].content = "http://nomand.co/content/images/"+ db[target].HEAD;
    }
  }
  else {
  ...
  $try(function(){content += `<div id="presentation">${new Runic(db[target].LONG, Curlic) + related}</div>`});
